...
export function DrawerContent({ className, children, ...props }: Omit<React.ComponentProps<typeof BaseDrawer.Popup>, "className"> & { className?: string }) {
  return <BaseDrawer.Portal><BaseDrawer.Backdrop className="lex-drawer__backdrop" /><BaseDrawer.Viewport className="lex-drawer__viewport"><BaseDrawer.Popup className={cx("lex-drawer__popup", className)} {...props}><BaseDrawer.Content className="lex-drawer__content">{children}</BaseDrawer.Content><BaseDrawer.Close className="lex-drawer__close" aria-label="Fechar"><X size={18} /></BaseDrawer.Close></BaseDrawer.Popup></BaseDrawer.Viewport></BaseDrawer.Portal>
}
