...
- `DropdownMenu`: lista compacta de ações sobre um item. Em `DropdownMenuContent`, use `align="start" | "center" | "end"` para fixar a âncora; em botões de ação do fim da linha (ícone de três pontos) prefira `align="end"` para o menu não ultrapassar a borda.
