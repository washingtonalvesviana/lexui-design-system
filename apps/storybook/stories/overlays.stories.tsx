...
const offcanvasPositions = [
  ["start", "Lateral esquerda"],
  ["end", "Lateral direita"],
  ["top", "Superior"],
] as const

export const OffcanvasPosicoes: Story = {
  render: () => (
    <div className="lex-story-stack">
      {offcanvasPositions.map((entry) => {
        ...
        return (
          <Drawer key={position}>
            ...
            <DrawerContent position={position}>
              <DrawerHeader><DrawerTitle>Painel {label.toLowerCase()}</DrawerTitle><DrawerDescription>Offcanvas com a posição selecionada; vira bottom sheet no mobile.</DrawerDescription></DrawerHeader>
              <DrawerFooter><DrawerClose render={<Button />}>Concluir</DrawerClose></DrawerFooter>
            </DrawerContent>
          </Drawer>
        )
      })}
