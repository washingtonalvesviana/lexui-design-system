...
import { CalendarPlus, Gavel, Handshake, Hourglass, MoreHorizontal, SlidersHorizontal, Trash2 } from "lucide-react"
import {
...
const eventKinds = {
  "Audiência": { icon: Gavel, variant: "accent" },
  "Prazo": { icon: Hourglass, variant: "warning" },
  "Acordo": { icon: Handshake, variant: "success" },
} as const

type EventKind = keyof typeof eventKinds

const events: { time: string; title: string; owner: string; kind: EventKind }[] = [
  { time: "09:00", title: "Audiência de conciliação", owner: "Ana Lima", kind: "Audiência" },
  { time: "11:30", title: "Prazo final de contestação", owner: "Rafael Costa", kind: "Prazo" },
  { time: "15:00", title: "Assinatura do acordo", owner: "Marina Souza", kind: "Acordo" },
]
...
export default function CalendarPage() {
  ...
  return <>
    ...
    <header className="demo-page-header demo-calendar-heading"><div><p className="demo-eyebrow">Planejamento</p><h1>Agenda</h1><p>Prazos, audiências e acordos do escritório em um só lugar.</p></div><Drawer><DrawerTrigger render={<Button />}><CalendarPlus size={17} /> Novo evento</DrawerTrigger><DrawerContent><DrawerHeader><DrawerTitle>Novo evento</DrawerTitle><DrawerDescription>Use o formulário padrão para manter datas e responsáveis consistentes.</DrawerDescription></DrawerHeader><div className="demo-form demo-drawer-form"><Field label="Título" htmlFor="event-title"><Input id="event-title" placeholder="Ex.: Audiência de instrução" /></Field><div className="demo-field-grid"><Field label="Data" htmlFor="event-date"><DatePicker value={date} onValueChange={setDate} /></Field><Field label="Tipo" htmlFor="event-type"><Select id="event-type"><option>Audiência</option><option>Prazo</option><option>Acordo</option></Select></Field></div></div><DrawerFooter><DrawerClose render={<Button variant="outline" />}>Cancelar</DrawerClose><DrawerClose render={<Button onClick={() => toast({ title: "Evento criado", description: "O compromisso foi adicionado à agenda.", variant: "success" })} />}>Criar evento</DrawerClose></DrawerFooter></DrawerContent></Drawer></header>
    <div className="demo-calendar-layout"><Card><CardHeader><CardTitle>Calendário</CardTitle><CardDescription>Selecione uma data para consultar os compromissos.</CardDescription></CardHeader><CardContent><Calendar value={date} onValueChange={setDate} /></CardContent></Card><Card><CardHeader><div className="demo-plan-heading"><div><CardTitle>Hoje</CardTitle><CardDescription>3 compromissos programados</CardDescription></div><Popover><PopoverTrigger render={<Button variant="outline" size="icon" aria-label="Filtrar agenda" />}><SlidersHorizontal size={17} /></PopoverTrigger><PopoverContent><PopoverTitle>Filtrar agenda</PopoverTitle><PopoverDescription>Os filtros persistem durante esta sessão.</PopoverDescription><div className="demo-form demo-popover-form"><Field label="Responsável" htmlFor="owner"><Select id="owner"><option>Ana Lima</option><option>Rafael Costa</option><option>Marina Souza</option></Select></Field></div></PopoverContent></Popover></div></CardHeader><CardContent><div className="demo-agenda">{events.map((event) => <article className="demo-agenda__item" key={event.time}><time>{event.time}</time><div><strong>{event.title}</strong><span>{event.owner}</span></div><AgendaKind kind={event.kind} /><DropdownMenu><DropdownMenuTrigger className="demo-icon-trigger" aria-label={`Ações de ${event.title}`}><MoreHorizontal size={18} /></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuItem>Editar evento</DropdownMenuItem><DropdownMenuItem>Duplicar</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem data-danger onClick={() => setDeleteEvent(event.title)}><Trash2 size={15} /> Excluir</DropdownMenuItem></DropdownMenuContent></DropdownMenu></article>)}</div></CardContent></Card></div>
...
function AgendaKind({ kind }: { kind: EventKind }) {
  const { icon: Icon, variant } = eventKinds[kind]
  return <Badge variant={variant}><Icon size={13} />{kind}</Badge>
}
